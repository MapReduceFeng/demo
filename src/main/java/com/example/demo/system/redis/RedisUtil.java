...
public class RedisUtil {
    ...
    @Autowired
    private StringRedisTemplate stringRedisTemplate_0;
    @Autowired
    private StringRedisTemplate stringRedisTemplate_1;
    @Autowired
    private StringRedisTemplate stringRedisTemplate_2;


    @PostConstruct
    public void init() {
        LOGGER.info(" RedisUtil init");
    }

    @PreDestroy
    public void destroy() {
        LOGGER.info(" RedisUtil destroy");
    }

    public void switchRedis(RedisDB redisDB) {
        switch (redisDB.dbId().getId()) {
            case 0:
                stringRedisTemplate = stringRedisTemplate_0;
                break;
            case 1:
                stringRedisTemplate = stringRedisTemplate_1;
                break;
            case 2:
                stringRedisTemplate = stringRedisTemplate_2;
                break;
            default:
                stringRedisTemplate = stringRedisTemplate;
        }

        LettuceConnectionFactory requiredConnectionFactory = (LettuceConnectionFactory) stringRedisTemplate.getRequiredConnectionFactory();
        LOGGER.info("切换 redis: {}", redisDB.dbId().getId());
        requiredConnectionFactory.setDatabase(redisDB.dbId().getId());
        // requiredConnectionFactory.setPort(6379);
        // requiredConnectionFactory.setHostName("127.0.0.1");
    }
